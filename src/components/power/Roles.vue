...
<!--        pull左移 push右移动-->
      ...
      <!--        角色列表区域-->
      <el-table :data="rolesList" style="width: 100%" border stripe size="mini">
<!--        展开列-->
...
<!--                通过for循环渲染二级权限-->
                <el-row :key="item2.id" v-for="(item2, i2) in item1.children"
                  ...
                  <el-col :span="18">
                    <el-tag :key="item3.id" v-for="(item3) in item2.children" type="warning" closable
        ...
        <el-table-column label="操作" >
          <template slot-scope="scope">
            ...
            <el-button type="warning" icon="el-icon-setting" size="mini"
                       @click="showSittingRightDialog(scope.row)">分配权限</el-button>
          </template>
        </el-table-column>
      </el-table>
    </el-card>
...
export default {
  ...
  methods: {
    ...
    async showSittingRightDialog (role) {
      this.roleId = role.id
      // 获取所有权限的数据
    ...
    // 通过递归的形式获取角色下所有三级权限的id并保存到数组
    getLeafKeys (node, arr) {
      // 如果node节点不包含children属性是三级节点
      if (!node.children) {
        // 把三级节点id保存到arr
